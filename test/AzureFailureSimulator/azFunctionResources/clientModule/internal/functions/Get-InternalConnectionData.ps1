...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'AzureFailureSimulator.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'AzureFailureSimulator.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'AzureFailureSimulator.Client.ProtectedToken'
