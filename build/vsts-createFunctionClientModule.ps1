...
$parentModule = 'AzureFailureSimulator'
if (-not $ModuleName) { $ModuleName = 'AzureFailureSimulator.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\AzureFailureSimulator'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\AzureFailureSimulator.psd1"
$prereqHash = @{
